    ...
    public class PrestitiRepository(string? connectionString)
    {
        ...
        public Prestiti? GetByIdLibro(int id)
        {
            string query = "SELECT * FROM Prestiti WHERE IDL = @id";
            var parameters = new[] { new SqlParameter("@id", id) };
            using var reader = _database.GetExecuteReader(query, parameters);
            if (reader.Read())
            {
        ...
        public List<Prestiti> GetByIdUtente(int id)
        {
            string query = "SELECT * FROM Prestiti WHERE IDU = @id";
            var parameters = new[] { new SqlParameter("@id", id) };
            var prestiti = new List<Prestiti>();
        ...
        {
            string query = "DELETE FROM Prestiti WHERE IDL = @id";
            var parameters = new[] { new SqlParameter("@id", id) };
            return _database.ExecuteNonQuery(query, parameters);
        }
    }
